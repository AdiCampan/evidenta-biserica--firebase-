...
function Copil({ childUpdated, removeChild, selected, persoane }) {
  ...
  useEffect(() => {
    // if (isLoading === false) {
    setDataNasterii(
      persoane?.find((persoana) => persoana.id === selected)?.birthDate
    );
    setVarsta(
      persoane?.find((persoana) => persoana.id === selected)?.birthDate
    );
    setSex(
      persoane?.find((persoana) => persoana.id === selected)?.sex ? "M" : "F"
    );
    // }
  }, [persoane]);
  ...
  const onCopilChange = (person) => {
    if (person.length > 0) {
      ...
      if (person[0].birthDate) {
        setDataNasterii(person[0].birthDate);

        // calcul varsta
        // const dataNasteriiUnix = Math.floor(
        //   new Date(person[0].birthDate).getTime() / 1000
        // );
        // const dataCurentaUnix = Math.floor(new Date().getTime() / 1000);
        // setVarsta(
        //   Math.floor((dataCurentaUnix - dataNasteriiUnix) / 3600 / 24 / 365)
        // );
        setVarsta(person[0].birthDate);
      }
  ...
  return (
    <tr>
      ...
      <td>{formatDate(dataNasterii)}</td>
      <td>{calculateAge(varsta)}</td>
      <td>{sex}</td>
    </tr>
  );
}
